...
def _align_skin(handler, dev_skin):
    """设备上报的皮肤与服务器配置不一致时，自动对齐（复用 admin 的写配置）

    ★ 为什么需要：设备按电源键切皮肤只写自己的 NVS，不通知服务器。
      OTA 是设备→服务器的现成通道，设备在 board 段上报皮肤
      （见设备侧 GetBoardJson）。
    ★ 为什么复用 AdminHandler._write_cfg：它自带「序列化校验 + 关键字段
      校验 + 备份 + 复核」，比在这里重写一份安全得多。
    """
    try:
        from core.api.admin_handler import AdminHandler, CONFIG_PATH
        import io as _io
        import yaml as _yaml

        with _io.open(CONFIG_PATH, encoding="utf-8") as f:
        ...
        if old == dev_skin:
            return
        hw["skin"] = dev_skin
        # 复用一个临时 AdminHandler 实例来走成熟的安全写流程
        ah = AdminHandler(handler.config, handler.logger)
        ah._write_cfg(cfg)
        handler.logger.bind(tag=TAG).info(
            "皮肤自动对齐（设备 OTA 上报）：{} → {}".format(old, dev_skin))
    except Exception as e:
        handler.logger.bind(tag=TAG).warning(
            "皮肤对齐失败（忽略）：%s", repr(e))
...
class OTAHandler(BaseHandler):
    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            # ★★★ 皮肤自动对齐（2026-09-19）
            #   修用户反馈：「按电源键切皮肤，服务器没跟着切」
            #
            #   机制：设备每次连服务器都走 OTA，携带 system_info JSON，
            #         其中 "board" 段里有 "skin"（设备侧 GetBoardJson 加的）
            #   ⇒ 与本地配置不一致就落盘对齐
            #   ⇒ 无论电源键/语音哪条路径切皮肤，设备一上线服务器就知道
            #
            #   ⚠️ 坑（我第一版踩的）：这段曾被我插进
            #      `if not device_model:` 分支里 —— 而真实设备【带了
            #      device-model 头】⇒ 该分支不进 ⇒ 对齐永不执行。
            #      ⇒ 必须放在【无条件执行】的位置。
            try:
                _bd = data_json.get("board") if isinstance(data_json, dict) else None
                if isinstance(_bd, dict):
                    _ds = (_bd.get("skin") or "").strip().lower()
                    if _ds in ("fairy", "geometry"):
                        _align_skin(self, _ds)
