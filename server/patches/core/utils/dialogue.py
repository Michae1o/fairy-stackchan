...
class Dialogue:
    ...
    def _log_chat(self, message: Message):
        """把【真实对话】落到 Web 控制台的对话记录里。

        ★ 必须过滤的三类，否则记录会被污染：
          · is_temporary=True  → few-shot 示例（"给我讲个故事吧"等）
          · role == "system"   → 人设提示词
          · role == "tool"     → 工具调用中间结果
        另外 content 为空（纯 tool_calls）的也跳过。
        ★ 任何异常都不能冒泡 —— 记录失败不能影响设备说话。
        """
        if chat_log is None:
            return
        try:
            ...
            if not message.content:
                return
            chat_log.record(message.role, message.content)
        except Exception:
            pass
