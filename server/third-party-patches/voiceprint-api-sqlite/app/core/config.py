...
class Settings:
    ...
    def sqlite(self) -> Dict[str, Any]:
        ...
        ★ 自改 2026-09-22：原版用 MySQL 存声纹特征，需要额外跑一个
          MySQL/MariaDB 服务。本项目实际只存几个声纹特征向量
          （每人几百字节），SQLite 完全够用，且能省掉一个常驻服务。
          配置项：sqlite.path（默认 data/voiceprints.db，表自动创建）
        """
