...
class VoiceprintDB:
    ...
    ★ 自改 2026-09-22：SQL 方言从 MySQL 改为 SQLite
      （占位符 %s → ?；ON DUPLICATE KEY UPDATE → ON CONFLICT DO UPDATE）。
      逻辑、日志、返回值一律保持原样，调用方无需改动。
    """
    ...
    def save_voiceprint(self, speaker_id: str, emb: np.ndarray) -> bool:
        ...
        try:
            with db_connection.get_cursor() as cursor:
                # ★ SQLite UPSERT（原为 MySQL 的 ON DUPLICATE KEY UPDATE）
                sql = """
    ...
    def delete_voiceprint(self, speaker_id: str) -> bool:
        ...
        try:
            with db_connection.get_cursor() as cursor:
                sql = "DELETE FROM voiceprints WHERE speaker_id = ?"  # ★ SQLite 占位符
