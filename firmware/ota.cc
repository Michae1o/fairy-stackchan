...
#include "settings.h"
#include "boards/m5stack/core-s3/skin_manager.h"
...
std::string Ota::GetCheckVersionUrl() {
    ...
    // ★★★ 自动记住「自建服务器地址」（2026-09-19）
    //   修用户反馈：「我刷了上一个固件，fairy皮肤依旧没指向我自己的服务器」
    //
    //   真因：切回 Fairy 皮肤时要读回自建 OTA 地址（见 skin_manager），
    //         而它优先读 fairy_skin/self_ota。
    //         若那里是空的、且 CONFIG_OTA_URL 又等于官方地址
    //         （开源版默认就是官方），切回 Fairy 就会【连官方】。
    //
    //   修法：设备每次 OTA 都会走这里取地址 ⇒ 若取到的是自定义地址，
    //         就把它记进 fairy_skin/self_ota（零配置、不写死 IP）。
