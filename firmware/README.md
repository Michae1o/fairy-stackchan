...
| 2 | 网页控制台切皮肤 | 已连上**你的**服务器（否则打不开控制台） |
| 3 | 切皮肤时自动记住 | 已连上你的服务器 |
...
方式 B（不编译，但必须先连一次「别的服务器」做中转）：
    设备先用通用固件连上官方服务器，再对它说：
      「我的服务器是 http://<你的IP>:8003/xiaozhi/ota/」
    ⇒ AI 调设备的本地 MCP 工具 self.server.set_ota_url，
      把地址直接写进设备 NVS（重启后生效）
    ⚠️ 前提：官方服务器可用（离线环境走不通）
    ⚠️ 用嘴念 IP 很难念对（ASR 常听错），建议一个数字一个数字念
```

> ⛔ **注意：不存在「在 Web 控制台里填服务器地址」这条路** ——
> 控制台是**你自建服务器**上的页面，设备连不上你的服务器就打不开它。
> 早期文档写过这个说法，是错的，现已修正。

> ⚠️ **改了 `server.http_port` 的用户请走方式 A**：
> 设备侧推导 OTA 地址时**固定按 8003**（`skin_manager.cc`），
> 不会读你的配置。
