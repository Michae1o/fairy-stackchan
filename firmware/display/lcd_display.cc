...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_flex_align(left_icons, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER,
                          LV_FLEX_ALIGN_CENTER);

    // ① 电量（最左）
    battery_label_ = lv_label_create(left_icons);
    ...
    //   ⇒ 状态栏里的 ☰ 菜单按钮【已删除】。
    //   状态栏排布回归：左边 WiFi（network_label_ 在左容器）
    //                   右边电量（battery_label_ 在 right_icons）
    //   待机转头 / 舵机跟随 改用【语音控制】（MCP 工具，已注册）
...
void LcdDisplay::SetupUI() {
    ...
    // Status bar starts hidden; a touch reveals it for 3s (ShowTopBarTemporarily).
    lv_obj_add_flag(top_bar_, LV_OBJ_FLAG_HIDDEN);

    // ★ 用户要求的状态栏排布：
    //   [电量] [WiFi] ............ [☰ 菜单]
    //   ⚠️ 注意：本块在 #else 分支（CONFIG_USE_WECHAT_MESSAGE_STYLE=n）
    //     —— 这才是本机实际编译进去的那套 SetupUI！
    //     之前我改到了 #if 分支（L420 附近），编译不进去 ⇒ UI 没变
    lv_obj_t* left_icons = lv_obj_create(top_bar_);
    lv_obj_set_size(left_icons, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
    lv_obj_set_style_bg_opa(left_icons, LV_OPA_TRANSP, 0);
    lv_obj_set_style_border_width(left_icons, 0, 0);
    lv_obj_set_style_pad_all(left_icons, 0, 0);
    lv_obj_set_flex_flow(left_icons, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(left_icons, LV_FLEX_ALIGN_START,
                          LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);

    // ★ 用户要求（2026-09-19）：「把状态栏回归左边wifi，右边电量」
    //   ⇒ 左容器 = WiFi；右容器 = 电量（+静音）
    network_label_ = lv_label_create(left_icons);
    lv_label_set_text(network_label_, "");
    lv_obj_set_style_text_font(network_label_, icon_font, 0);
    ...
    lv_obj_set_style_bg_opa(bottom_bar_, LV_OPA_40, 0);   // semi-transparent overlay
    lv_obj_set_style_radius(bottom_bar_, lvgl_theme->spacing(4), 0);  // rounded corners
    lv_obj_set_style_bg_color(bottom_bar_, lvgl_theme->background_color(), 0);
    lv_obj_set_style_text_color(bottom_bar_, lvgl_theme->text_color(), 0);
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    if (bottom_bar_ != nullptr) {
        if (content == nullptr || content[0] == '\0') {
            ...
            // Fade the overlay in (200ms) so it doesn't pop abruptly.
            lv_anim_delete(bottom_bar_, nullptr);
            lv_obj_set_style_opa(bottom_bar_, LV_OPA_TRANSP, 0);
            lv_anim_t fade;
            lv_anim_init(&fade);
            lv_anim_set_var(&fade, bottom_bar_);
...
// ★ 把常显的 ☰ 提到最顶层
//   为什么需要：菜单遮罩 menu_root_ 是【全屏 + CLICKABLE】，
//   打开菜单时会 lv_obj_move_foreground(menu_root_) 把它提到最前
//   ⇒ 若不把 ☰ 也提上来，菜单打开后 ☰ 被盖住 ⇒ 关不掉菜单
void LcdDisplay::RaiseMenuButton() {
    DisplayLockGuard lock(this);
    if (menu_float_btn_ != nullptr) {
        lv_obj_move_foreground(menu_float_btn_);
    }
}

void LcdDisplay::ShowTopBarTemporarily() {
    DisplayLockGuard lock(this);
    if (top_bar_ == nullptr) {
        return;
    }
    ...
    if (top_bar_timer_ != nullptr) {
        lv_timer_delete(top_bar_timer_);
        top_bar_timer_ = nullptr;
    }
    top_bar_timer_ = lv_timer_create(
        [](lv_timer_t* t) {
            auto* self = static_cast<LcdDisplay*>(lv_timer_get_user_data(t));
            if (self != nullptr && self->top_bar_ != nullptr) {
                lv_obj_add_flag(self->top_bar_, LV_OBJ_FLAG_HIDDEN);
            }
            self->top_bar_timer_ = nullptr;
            lv_timer_delete(t);
        },
        3000, this);
}
