...
esp_err_t I2cDevice::TryReadRegs(uint8_t reg, uint8_t* buffer, size_t length) {
    ...
    // 超时放宽到 200ms（100ms 在 Fairy 皮肤解 GIF 时偶发不够）
    return i2c_master_transmit_receive(i2c_device_, &reg, 1, buffer, length, 200);
}

esp_err_t I2cDevice::TryReadReg(uint8_t reg, uint8_t* out) {
    return TryReadRegs(reg, out, 1);
}
