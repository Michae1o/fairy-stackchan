...
为什么：设备本身零智能，语音识别 / 大模型 / 音色 / 人设全在服务器那边。
        没有服务器 ⇒ 它只能连官方服务器 ⇒ 你能对话，
        但没有你自己的音色、没有你的 Fairy 人设、没有网页控制台。

⇒ 所以正确顺序是【先服务器，再刷设备】：
     ① 起服务器：INSTALL.md §1（含判据：/admin 返回 200 + test_server_e2e.py 四个 ✅）
     ② 刷本页的固件：第二节
     ③ 让设备连过来：第三节（配网页填你自己的服务器地址）

★ 反过来说：如果你**不想自建服务器**，那就只能连官方服务器 ——
  设备能亮、能对话（官方的音色和人设），固件侧的互动（表情/摸头/甩晕）也还在，
  但**你自己的**音色、人设、网页控制台就都没有了。
```
...
#### 路径 A（要编译，本页不展开）：把地址编进固件 ⇒ 刷完**只需配网**

适合「给朋友用」：编译时写死地址，他刷完配个网就能连、什么都不用配。
...
判据：刷完配好网，串口【直接】出现 WS: Connecting to ws://<你的IP>:8000/…
      而你没有在配网页填过任何地址
```

⚠️ 写死地址的固件**别公开分发**（会泄露你的内网 IP）。
详细做法与优先级说明（编进去的是默认值，配网页填的会覆盖它）见
[../INSTALL.md](../INSTALL.md) §2.4.2 与 §3 方法 B。
...
✔ 2 个表情动画 GIF（约 2 MB，作者自绘的二次创作）
    ⇒ 这就是它能显示全屏表情的原因
    ⇒ ★ 只在这个预编译固件里有；仓库源码包【不含任何表情/角色素材】
      （源码包里唯一的图是 docs/console-home.png，那是控制台界面截图、文档配图）
