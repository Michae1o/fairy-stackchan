...
```
① 微调权重单个 148 MB，超出 GitHub 单文件 100 MB 限制
② 音色版权归属【原声者 / 训练者】，公开再分发有风险
⇒ 所以本仓库只放【教程】，文件请你自行准备
```

**来源（音色参考）**：B 站 UP 主「白菜工厂1145号员工」
空间：<https://space.bilibili.com/518098961>

```
· 参考音频：从上面这位 UP 主的公开视频里取一段清晰人声
            （5~10 秒即可，取法见 ../audio/README.md）
· 微调权重：其实【不需要微调】——
            GPT-SoVITS 的零样本（zero-shot）用参考音频就能克隆音色，
            效果已经很好（本项目线上就是这么用的）
            想更稳再按第 3 节跑一遍微调
```

> ⚠️ 使用任何人的音色前请注意：
> 请勿用于冒充他人、商业用途、违法内容；若原作者要求下架应立即停止。
...
TTS:
  GPTSoVITSTTS:
    ...
    ref_audio_path: <绝对路径>/voice-package/reference.wav
    # ★ 换微调模型时，这三项必须同步改，否则音色走样：
