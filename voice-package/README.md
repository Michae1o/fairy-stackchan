...
>
> 💡 想商用？→ 换成【你自己的声音】录参考音频（零版权顾虑，见 §5.1 办法 A）。
...
> 不确定音频说了什么的话，先在 WebUI 里用「ASR 打标」把参考音频转成文字（见 §3.1）。
...
> 本节原来是一份独立文档（`audio/README.md`），已并入这里 —— 音色的事只看这一份。

### 5.1 三条获取途径

| 途径 | 怎么做 | 版权 |
|---|---|---|
| **A. 录你自己的声音**（★ 推荐） | Windows「录音机」录一句完整的话，如「今天天气不错，我们一起出去走走吧」；安静房间、离嘴 20cm、平常语气 | 你自己的，可商用 |
| B. 用网盘里带的 | 见 §0 的网盘渠道（UP 主分享） | 归原声者，别二次分发 |
| C. 从公开视频里截 | `yt-dlp "<视频URL>"` → `ffmpeg -i input.mp4 -vn -ac 1 -ar 48000 ref_raw.wav` → 剪出 5~10 秒纯人声 | 同上，注意别用 BGM 混音段 |

> 视频里人声和 BGM 混在一起要用 UVR5 / demucs 分离；
> **能换一段没 BGM 的素材就换**，比分离出来的效果好。
> 批量做的话 GPT-SoVITS 自带切片脚本（`tools/` 下）能把长音频自动切成多段再挑。
...
| 采样率 | 32k / 44.1k / 48k | 都行，不必重采样 |
| 声道 | 单声道 | 立体声也能用，单声道更稳 |
| 格式 | `.wav` | mp3/m4a 建议先转 wav |
| 环境 | 安静、**无 BGM、无混响** | ★ 最重要 —— BGM 会被当成「音色」一起学进去 |

三个最常见的坑：① 截到带 BGM 的片段 ⇒ 合成带「噪音音色」② 截到多人对话 ⇒ 音色混
③ 片段里有笑声/叹气 ⇒ 语气飘。**选「一个人、安静、平稳说一整句」的最好。**

### 5.3 放哪里（★ 服务器会去哪个目录找）

服务器代码算出来的参考音频目录是 **`<服务器 clone 的上一级>/fairy-ref/`**：
例：服务器 clone 在 `/opt/xiaozhi-esp32-server` ⇒ 音频放 **`/opt/fairy-ref/`**。

```text
⛔ 不是「与 main/ 同级」—— 代码从 core/api/ 上溯 3 层（得到 <clone>/main/xiaozhi-server）
   再上一级，所以比 clone 根目录还高一层。
⇒ 控制台的「音色」页会从这个目录列出可选音频（放这里才能下拉选）
⇒ 放别处也能用，但要手填绝对路径
```

### 5.4 没有参考音频能跑吗

**能跑，但音色不是「那个角色」** —— 不填 `ref_audio_path` 时用的是预置通用音色。
想要特定音色，必须给参考音频（零样本）或微调权重（见 §2）。

### 5.5 使用限制（用别人音色时）

```text
① 音色版权归属【原声者 / 训练者】，不属于本项目作者
② 用别人的音色时请勿：冒充原声者、商业用途、违法内容
③ 原权利人要求下架 ⇒ 立即停止
④ 二次分发时保留来源标注
★ 这一条限制的是【音色 / 音频文件】；本项目【代码】是 MIT，商用没问题，别搞混。
```
