...
- **网页控制台**：PC 版 `/admin`（状态灯、硬件开关、皮肤、TTS 试听、网页对话、**声纹管理**），手机版 `/m` —— 用法见 [`CONSOLE.md`](CONSOLE.md)
- **声纹识别**：设备每次说话都比对声纹，认得出「是谁在说」；控制台「声纹」页能直接**加人 + 用网页录音注册**（声纹服务用自建的 [`voiceprint-api`](https://github.com/xinnan-tech/voiceprint-api)，接入见 [`server/README.md`](server/README.md)）
...
| | 内容 |
|---|---|
| **有** | 上游**改动源码**（固件：板卡层 / 头像与装饰器 / 本地组件 / 显示层；服务器：10 个文件）· **9 个工具脚本** · 参数与做法文档 · 1 张控制台界面截图（`docs/`）· 免编译固件（在 Release 里） |
